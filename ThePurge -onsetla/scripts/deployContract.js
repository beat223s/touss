...
async function main() {
  ...
  // Deploy the contract
  const SavetheVillagers = await hre.ethers.getContractFactory('SavetheVillagers,')
  const savethevillagers = await SavetheVillagers.deploy(
    BASE_URI,
    root,
    proxyRegistryAddressRinkeby
  )

  await savethevillagers.deployed()

  console.log('SavetheVillagers deployed to:', savethevillagers.address)
}
